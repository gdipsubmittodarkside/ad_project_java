...
public class RMyCourseController {
    ...
    public ResponseEntity<Boolean> saveMyCourse(
            @PathVariable(value = "member_id") Long member_id,
            @PathVariable(value = "courseCrawled_id") Long course_id) {

        Member member = memberService.findById(member_id);
        CourseCrawled courseCrawled = courseCrawledService
                .findCourseCrawledById(course_id);
    ...
    public ResponseEntity<List<CourseCrawled>> getMyCourseByMemberId(
            @PathVariable("member_id") Long member_id) {

        // should add json ignore for fields scheduleEvent and member for MyCourse
        // entity, such that there won't be nested json.
        /*
         * currently this is not so important as we are not returning a list of
         * MyCourse. this is because mobile side need to display all the fields of
         * course crawled for saved course page
         */
        List<MyCourse> myCourseList = myCourseService
                .getMyCoursesByMemberId(member_id);

        List<CourseCrawled> listOfCClinkedToMyCourses = myCourseList
                .stream()
                .map(c -> courseCrawledService
                        .findCourseCrawledByUrlAndSkillId(
                                c.getCourseUrl(),
                                c.getSkill()))
                .collect(Collectors.toList());

        try {
            if (listOfCClinkedToMyCourses == null
                    || listOfCClinkedToMyCourses
                            .size() == 0) {
                return new ResponseEntity<>(
                        HttpStatus.NO_CONTENT);
            }
    ...
    public ResponseEntity<Long> deleteMyCourse(
            @PathVariable("id") Long id) {

        try {
            var isRemoved = myCourseService
                    .deleteMyCourse(id);
            if (!isRemoved) {
                return new ResponseEntity<>(
                        HttpStatus.NO_CONTENT);
