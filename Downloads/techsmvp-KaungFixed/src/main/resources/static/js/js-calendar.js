var cal = {
  ...
  hYear: "", // month & year
  hCD: "",
  hCB: "", // calendar days & body
  hFormWrap: "",
  hForm: "", // event form
  hfID: "",
  hfStart: "", // event form fields
  hfEnd: "",
  hfTxt: "",
  hfColor: "",
  hfBG: "",
  ...
  load: () => {
    ...
    cal.sL = parseInt(
      String(cal.sYear) + String(m) + String(cal.sDIM) + "2359"
    );
  ...
  show: (id) => {
    if (id) {
      ...
      if (cal.course.options.length !== 0 && cal.myCoursesS !== null) {
        for (let i = 0; i < cal.course.options.length; i++) {
          ...
          if (
            cal.myCoursesS.innerHTML.indexOf(cal.course.options[i].value) !== -1
          ) {
  ...
  save: () => {
    ...
    if (cal.hfID.value != "") {
      data.id = parseInt(cal.hfID.value);
    }
    console.log(data);
