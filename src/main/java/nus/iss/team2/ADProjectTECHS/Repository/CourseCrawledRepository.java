...
public interface CourseCrawledRepository extends JpaRepository<CourseCrawled, Long> {
    ...
    @Query("select c from CourseCrawled c where c.skill.skillId = :skillId order by c.likes desc")
    List<CourseCrawled> findCCByOrderByLikes(@Param("skillId") long skillId);

    @Query("select c from CourseCrawled c where c.skill.skillId = :skillId order by c.views desc")
    List<CourseCrawled> findCCByOrderByViews(@Param("skillId") long skillId);

    @Query("select c from CourseCrawled c where c.skill.skillId = :skillId order by c.date desc")
    List<CourseCrawled> findCCByOrderByDate(@Param("skillId") long skillId);

    @Query("select c from CourseCrawled c where c.skill.skillId = :skillId order by c.subscribers desc")
    List<CourseCrawled> findCCByOrderBySubscribers(@Param("skillId") long skillId);
